...
if __name__ == "__main__":
    ...
    wgrn05_base_path = "../../cloudcontainer/Parameters_downloaded_on_jan5/wgrn05/alt"

    # List all file names in alt directory:
    all_files_list = os.listdir(wgrn05_base_path)
    print("All files: ", all_files_list)

    for found_file in all_files_list:
        file_path = f"{wgrn05_base_path}/{found_file}"
        ...
        for dataset_path in all_datasets_list:
            ...
            # Because there was a clipping mistake in the original files I need to preprocess:
            preprocessed_time_column = features_dict[dataset_path][0].apply(lambda x: math.modf(x)[1] if math.modf(x)[0] < 0.5 else math.modf(x)[1] + 0.5)
            features_dict[dataset_path][0] = preprocessed_time_column
    ...
    df_final.to_hdf("../../cloudcontainer/wgrn05_features.hdf5", key='wgrn05', mode='w')