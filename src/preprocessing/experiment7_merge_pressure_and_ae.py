...
exp7_ae_df = pd.read_csv("../../cloudcontainer/Parameters_downloaded_on_jan5/wgrn07/ae_wgr07_150_axial.x_ae_02_aic.csv")

print(exp7_ae_df.head())

pressure_and_features_path = "../../cloudcontainer/wgrn07_features_and_pressure.hdf5"
pressure_and_features_df = pd.HDFStore(pressure_and_features_path, mode="r").select("wgrn07")
...
floored_ae_time = exp7_ae_df["t_dax0"].apply(lambda x: math.modf(x)[1] if math.modf(x)[0] < 0.5 else math.modf(x)[1] + 0.5)

exp7_ae_df["ae_floored_time"] = floored_ae_time

exp4_ae_df_aggregated = exp7_ae_df.groupby("ae_floored_time").agg({
    'adjamp':['mean']
}).reset_index()
...
ae_features_and_pressure_df.to_hdf("../../cloudcontainer/wgrn07_ae_features_and_pressure.hdf5", key='wgrn07', mode='w')